...
paginator = client.get_paginator('list_objects') # client.list_objects only goes to 1000
page_iterator = paginator.paginate(Bucket=bucket_name)

objects = [k for page in [[o for o in page["Contents"]] for page in page_iterator] for k in page]
get_last_modified = lambda obj: int(obj['LastModified'].strftime('%s'))
sorted_objects = [obj['LastModified'] for obj in sorted(objects, key=get_last_modified, reverse=True)]
...
get_last_modified = lambda obj: int(obj['LastModified'].strftime('%s'))
files = [obj.key for obj in sorted(bucket.objects.filter(), key=get_last_modified, reverse=True)][0:9]
